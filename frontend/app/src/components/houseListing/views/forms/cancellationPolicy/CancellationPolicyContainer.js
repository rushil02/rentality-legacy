...
export default class CancellationPolicyContainer extends Component {
    ...
    sync = () => {
        getCancellationPolicies(this.props.houseUUID).then(result => {
            this.setState(prevState => ({
                ...prevState,
                data: result,
            }))
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-10">
                            <h1 className={commonStyles.pageTitle}>
                                Which Cancellation Policy would you like to opt for?
                            </h1>
                            <div className="paragraph">
                                {this.state.data.getObjectList().map(item => (
                                    <CancellationPolicyComponent
                                        data={item[1]}
                                        key={item[0].toString()}
                                        onChange={this.onPolicyUpdate}
                                        objRef={item[0]}
                                        isSelected={
                                            this.state.house.getData("cancellationPolicyID") ===
                                            item[1].getData("objID")
                                        }
                                    />
                                ))}
                            </div>
                            ...
                            <div className="col-md-12" style={{ marginTop: "50px" }}>
                                <p>
                                    <b>
                                        Please note: The tenant is allowed to cancel the booking without penalty within
                                        24 hours of the booking being approved by the homeowner. (As required by
                                        legislation)
                                    </b>
                                </p>
                            </div>
