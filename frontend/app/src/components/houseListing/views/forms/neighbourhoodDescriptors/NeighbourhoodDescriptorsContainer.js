...
import {
    getHouseData,
    getNeighbourhoodDescriptors,
    patchHouseData,
    postNeighbourhoodDescriptors,
} from "../../../services"
...
export default class NeighbourhoodDescriptorsContainer extends Component {
    ...
    componentDidMount() {
        ...
        if (this.state.descriptors.status === "empty") {
            getNeighbourhoodDescriptors(this.props.houseUUID).then(result => {
    ...
    onNBDescriptorAdd = input => {
        ...
        if (text !== "") {
            this.setState(prevState => ({
                descriptors: prevState.descriptors.update(
                    new NeighbourhoodDescriptor({ id: null, verbose: text, checked: true }, "hasChanged"),
                    text
                ),
            }))
